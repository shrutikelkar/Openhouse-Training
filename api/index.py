...
async def explain_turn(req: Request, authorization: Optional[str] = Header(None)):
    ...
    system = (
        ...
        "or equipment the game needs, using the 'materials' field as the correct answer. Every "
        "named item in that list is required, physical or digital (an app, deck, or link — often "
        "marked '(app)') — the trainee must name it specifically for this to count as correct.\n"
        "- CRITICAL, checked at scoring time regardless of which questions got asked: go through "
        "the reference data's 'materials' list AND every distinct object/card/prop mentioned inside "
        "'steps' (e.g. a specific named card type that a step says gets drawn, given, or used) and "
        "check whether the trainee's own words actually named each one and how it's used. A trainee "
        "can describe the overall flow of the game fluently while quietly never mentioning one "
        "specific card or item the rules depend on (e.g. never explaining what a 'strike card' is "
        "or when a child gets one, while still describing everything else about a debate correctly). "
        "That is a real, specific gap — score gameplay_accuracy 0 for it and name exactly which item "
        "or mechanic was missing in \"improvements\". Do not award a pass on general fluency alone.\n"
        + (
        ...
        + (
            ...
            "including every specifically named item (physical or digital) that 'materials' or 'steps' calls "
            "for. A fluent overall explanation does not earn this if one such named item was never mentioned; "
            "check the list item by item rather than judging the explanation's general quality.\n"
        ...
        "  - \"improvements\": an array with one entry for EVERY criterion that scored 0. NEVER leave "
        "this empty, even if all five criteria scored 1 — a trainee who got everything right can still "
        "grow, so in that case include at least one entry naming a specific way to take an already-"
        "correct explanation further (sharper phrasing, a livelier hook, tighter pacing, a more vivid "
        "worked example, etc.), tied to whichever of the five criteria it would sharpen. Each entry: "
        '{"criterion":"<the criterion key>","reason":"<for a scored-0 criterion, the specific, concrete '
        "reason this point was lost, referencing what they actually said; for a scored-1 criterion used "
        "as a growth suggestion, what they already did well AND what the next-level version would look "
        'like>","fix":"<a specific, actionable way to improve — a concrete example to use, a tone '
        "adjustment, simpler wording, a particular step to add — not a vague \"explain more clearly\">\"}. "
        "Keep the tone constructive, never harsh, even though the reason is specific about the gap.\n\n"
        + (
